...
import torch
from model_architectures import SimplifiedResNet
import numpy as np
...
num_classes = 13   
model = SimplifiedResNet(input_shape, num_classes)

# Define the file path for the saved model weights
model_weights_path = 'Project-Clone/Model/SimpleResNet_weights.pth'
...
def test():
    ...
    for file_path in os.listdir(TEST_DATA_DIRECTORY_ABSOLUTE_PATH):
        prediction = evaluate(os.path.join(TEST_DATA_DIRECTORY_ABSOLUTE_PATH, file_path))
